...
const Task = ({ task, selectTask, classname }) => {
                ...
                <div className="flex items-center justify-between">
                    ...
                    <ul className="text-sm text-end">
                        ...
                        <li className="inline-block me-2 ms-2">
                            <span className={`${task.priority === 'High' ? 'bg-danger/20 text-danger' : task.priority === 'Medium' ? 'bg-info/20 text-info' : 'bg-success/20 text-success'} rounded text-xs/none align-baseline font-medium p-1`}>
                                {task.priority}
                            </span>
                        </li>
                    </ul>
                ...
                <div className="card p-6 mb-8">
                    {taskList.map((task, idx) => (
                        <Task 
                            ...
                            task={task} 
                            key={idx} 
                            classname={idx === taskList.length - 1 ? '' : 'mb-4'} 
                        />
                    ))}
                </div>
            </Collapse.Menu>
