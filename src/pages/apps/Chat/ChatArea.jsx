  ...
  return (
    <div className={`${message.from.id === toUser.id ? 'flex-row-reverse items-start text-end' : 'items-start text-start'} flex gap-3 group`}>
      <div className="text-center">
        <img src={message.from.avatar} className="rounded-md h-8" alt="Avatar" />
...
const ChatArea = ({ selectedUser, chatToggler }) => {
  ...
  const getMessagesForUser = useCallback(() => {
    if (selectedUser) {
      setTimeout(() => {
        setUserMessages([
          ...messages.filter((m) => (m.to.id === toUser.id && m.from.id === selectedUser.id) || (toUser.id === m.from.id && m.to.id === selectedUser.id)),
        ]);
      }, 750);
    }
  }, [selectedUser, toUser]);
